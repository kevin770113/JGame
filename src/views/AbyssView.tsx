  ...
  return (
    <div className="w-full flex flex-col gap-5 pb-32 animate-fade-in relative z-10">
      <div className="flex justify-between items-center border-b border-gray-700 pb-2">
        <div>
          <h2 className="text-xl font-bold text-purple-400">{t('abyss.title', '深淵之塔')}</h2>
          <p className="text-xs text-gray-500 mt-1">{t('abyss.desc', '無盡的黑暗階梯，埋藏著古代的英靈與無盡的財富。')}</p>
        </div>
        <button 
      ...
      <div className="flex flex-col md:flex-row gap-5 mt-2">
        <div className="w-full md:w-1/2 flex flex-col gap-3">
          <div className="bg-gray-900/80 p-5 rounded-lg border border-purple-900/50 shadow-xl relative overflow-hidden group flex flex-col items-center text-center">
            ...
            <div className="text-xs text-purple-500 font-bold tracking-widest mb-1">
              {targetEnemy.isBoss ? t('abyss.boss', '［鎮守英靈］') : t('abyss.monster', '［深淵魔物］')}
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 mt-2 bg-gray-950 p-3 rounded border border-gray-800 text-xs w-full text-left">
              <div className="flex justify-between"><span>{t('stats.combat', '武力')}:</span> <span className="text-red-400 font-bold font-mono">{targetEnemy.stats.combat}</span></div>
              <div className="flex justify-between"><span>{t('stats.endurance', '體質')}:</span> <span className="text-green-400 font-bold font-mono">{targetEnemy.stats.endurance}</span></div>
              <div className="flex justify-between"><span>{t('stats.intelligence', '智力')}:</span> <span className="text-blue-400 font-bold font-mono">{targetEnemy.stats.intelligence}</span></div>
              <div className="flex justify-between"><span>{t('stats.charisma', '魅力')}:</span> <span className="text-pink-400 font-bold font-mono">{targetEnemy.stats.charisma}</span></div>
              <div className="flex justify-between col-span-2"><span>{t('stats.luck', '幸運')}:</span> <span className="text-yellow-400 font-bold font-mono">{targetEnemy.stats.luck}</span></div>
            </div>
            ...
            <div className="mt-4 text-xs text-purple-400 tracking-widest bg-purple-950/20 px-4 py-2 rounded border border-purple-900/30 w-full text-center">
              {t('arena.expected_reward', '期望賞金:')} <strong className="text-yellow-500">${expectedReward}</strong> 
              {charismaBonusMultiplier > 1 && <span className="text-3xs text-gray-600 ml-1">{t('abyss.charisma_bonus', '(含魅力)')}</span>}
              {targetEnemy.rewardPrestige > 0 && <span className="ml-2">| {t('ui.prestige', '威望:')} <strong className="text-blue-400">+{targetEnemy.rewardPrestige}</strong></span>}
            </div>
          </div>
        </div>
        ...
        <div className="w-full md:w-1/2 flex flex-col gap-4 bg-gray-900/60 p-4 rounded-lg border border-gray-800 justify-between min-h-[220px]">
          <div className="flex flex-col gap-2">
            <label className="text-xs text-purple-400 font-bold tracking-widest border-l-2 border-purple-600 pl-2 mb-1">{t('abyss.dispatch_challenger', '［派遣挑戰者］')}</label>
          ...
          <div className="flex flex-col gap-3 mt-auto">
            <div className="text-xs text-gray-500 italic mt-1 leading-relaxed">
              ※ {t('abyss.warning_cost_1', '深淵挑戰將')} <strong className="text-yellow-500">{t('abyss.warning_cost_2', '強制消耗 1 點行動力推進時段')}</strong>{t('abyss.warning_cost_3', '，並消耗該挑戰者')} <strong className="text-purple-400">{t('abyss.warning_cost_4', '30 點體力')}</strong>。
            </div>
