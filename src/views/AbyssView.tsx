...
export default function AbyssView() {
  ...
  const [combatResult, setCombatResult] = useState<{ logs: CombatLog[], isWin: boolean } | null>(null);
  ...
  return (
    <div className="w-full flex flex-col gap-4 pb-10 animate-fade-in relative">
      ...
      <div className="flex flex-col gap-5 mt-2 relative z-10">
        {combatResult ? (
          <div className="bg-gray-900 border border-purple-900/50 rounded-lg p-4 flex flex-col gap-4 shadow-2xl shadow-purple-900/20">
             <h3 className={`text-lg font-bold tracking-widest text-center ${combatResult.isWin ? 'text-purple-400' : 'text-red-600'}`}>
               {combatResult.isWin ? '［成功鎮壓深淵］' : '［靈魂遭到吞噬］'}
             </h3>
             <div className="bg-gray-950 p-3 rounded border border-gray-800 h-64 overflow-y-auto font-mono text-xs flex flex-col gap-2 scrollbar-none shadow-inner">
               {combatResult.logs.map((log, idx) => (
                 <div key={idx} className={`${log.type === 'system' ? 'text-purple-400 italic border-b border-gray-900 pb-1' : log.type === 'skill' ? 'text-yellow-400 font-bold' : log.type === 'heal' ? 'text-green-400' : log.type === 'damage' ? 'text-red-400' : 'text-gray-300'}`}>
                   {log.round > 0 && <span className="text-gray-600 mr-2">R{log.round}</span>}{log.message}
                 </div>
               ))}
               <div ref={logEndRef} />
             </div>
             <button onClick={() => setCombatResult(null)} className="w-full py-2.5 bg-purple-900/40 hover:bg-purple-900/60 text-purple-200 border border-purple-700/50 rounded font-bold text-xs tracking-widest transition-colors">
               {combatResult.isWin ? '［繼續前往下一階層］' : '［回收殘骸並離開］'}
             </button>
          </div>
        ) : (
          <>
