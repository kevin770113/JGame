...
export default function InteractionView() {
  ...
  const appointRole = useGameStore((state) => state.appointRole); // ★ V2.6 職務任免綁定

  const [selectedSlaveId, setSelectedSlaveId] = useState<string>('');
  const [activeTab, setActiveTab] = useState<'dialogue' | 'clean' | 'train' | 'inventory' | 'role'>('dialogue'); // ★ V2.6 擴充 role 頁籤
  ...
  const slaveOptions: Option[] = idleSlaves.map(s => ({
    value: s.id,
    label: `${s.name} (體力: ${s.conditionStats.stamina}${s.role === 'maid' ? ' | 傭人' : s.role === 'security' ? ' | 保全' : ''}${s.faintTurns > 0 ? ' | 昏厥' : ''})`
  }));
  ...
  const executeClean = () => {
    ...
    // ★ V2.6 負傷狀態技能等級折半計算
    const effectiveHousework = selectedSlave.isInjured 
      ? Math.floor((selectedSlave.skills.housework || 1) * 0.5) 
      : selectedSlave.skills.housework;
    ...
    setSysMessage({ text: `［結算］${selectedSlave.name} 執行了環境整理。髒亂度大副下降。`, type: 'success' });
  };
  ...
  const isFainted = selectedSlave ? (selectedSlave.faintTurns || 0) > 0 : false;
        ...
        <div className="flex flex-col gap-4 mt-2 relative">
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none border-b border-gray-800">
            {[
              ...
              { id: 'clean', label: '［內政指派］' },
              { id: 'train', label: '［殘酷特訓］' },
              { id: 'inventory', label: '［道具裝備］' },
              { id: 'role', label: '［職務任免］' } // ★ V2.6 新增
            ].map(tab => (
              <button 
                key={tab.id} 
                onClick={() => setActiveTab(tab.id as any)} 
                className={`whitespace-nowrap px-4 py-2 text-xs font-bold transition-colors tracking-widest ${ activeTab === tab.id ? 'text-white border-b-2 border-blood-red' : 'text-gray-500 hover:text-gray-300' }`}
              >
                {tab.label}
              </button>
            ))}
          ...
          <div className="bg-gray-900/80 p-4 rounded-lg border border-gray-700 shadow-lg animate-fade-in min-h-[160px] relative">
            
            {/* ★ V2.6 昏厥狀態操作攔截網 */}
            {isFainted ? (
              <div className="absolute inset-0 bg-gray-950/90 z-30 flex flex-col items-center justify-center p-4 text-center backdrop-blur-xs rounded-lg">
                <span className="text-red-500 font-black tracking-widest text-sm animate-pulse border border-red-900/50 bg-red-950/20 px-4 py-2 rounded">
                  ［⚠️ 該成員目前處於昏厥狀態］
                </span>
                <p className="text-xs text-gray-500 mt-3 leading-relaxed max-w-xs">
                  該成員因體力徹底榨乾而陷入重度昏厥，目前尚需 <strong className="text-gray-300 font-mono">{selectedSlave.faintTurns}</strong> 個時段甦醒。在此期間無法進行任何互動管理與派遣。
                </p>
              </div>
            ) : null}
              ...
              <div className="flex flex-col gap-4">
                <div className="text-xs text-gray-400 bg-gray-950 p-3 rounded border border-gray-800">
                  <div className="flex justify-between mb-1">
                    <span>當前據點髒亂度：</span> 
                    <span className={roomDirtiness > 50 ? 'text-yellow-500' : 'text-green-500'}>{roomDirtiness}%</span>
                  </div>
                  <div className="flex justify-between">
                    <span>成員內政管家等級：</span> 
                    {selectedSlave.isInjured ? (
                      <span className="text-red-400 font-bold">
                        Lv.{Math.floor((selectedSlave.skills.housework || 1) * 0.5)} <span className="text-gray-600 font-normal text-2xs">(原: Lv.{selectedSlave.skills.housework})</span>
                      </span>
                    ) : (
                      <span className="text-blue-400">Lv.{selectedSlave.skills.housework}</span>
                    )}
                  </div>
                ...
                <button onClick={() => requestTimeSkipAction('下令整頓環境', executeClean)} className="w-full py-2.5 bg-gray-800 hover:bg-gray-700 text-gray-300 border border-gray-600 rounded font-bold text-xs tracking-widest transition-colors">
                  ［下令整頓環境］
                </button>
              </div>
            )}
                      ...
                      return (
                         <div key={itemId} className="flex justify-between items-center bg-gray-950 p-3 border border-gray-800 rounded shadow-sm">
                            ...
                            <button 
                              onClick={() => {
                                 if (item.type === 'potion') { 
                                   useItem(itemId, selectedSlave.id); 
                                   setSysMessage({ text: `［系統］${selectedSlave.name} 消耗了 ${item.name}。`, type: 'success' });
                                 } else { 
                                   equipWeapon(itemId, selectedSlave.id);
                                   setSysMessage({ text: `［系統］${selectedSlave.name} 裝備了 ${item.name}。`, type: 'success' }); 
                                 }
                              }} 
            ...
            {activeTab === 'role' && (
              <div className="flex flex-col gap-4 animate-fade-in">
                <div className="text-xs text-gray-400 bg-gray-950 p-3 rounded border border-gray-800 flex flex-col gap-1.5">
                  <div className="flex justify-between">
                    <span>原生服從度：</span> 
                    <span className={selectedSlave.primaryStats.obedience >= 80 ? 'text-green-400 font-bold' : 'text-red-400'}>
                      {selectedSlave.primaryStats.obedience} / 80
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span>目前商會職務：</span>
                    <span className="text-yellow-500 font-bold">
                      {selectedSlave.role === 'maid' ? '內務傭人' : selectedSlave.role === 'security' ? '商會保全' : '暫無職務'}
                    </span>
                  </div>
                  <div className="text-purple-400 text-2xs border-t border-gray-800 pt-2 mt-1 leading-relaxed">
                    ※ 傭人於每日深夜自動扣除據點髒亂度，但會強制耗費體力；保全則於深夜強制進行越獄車輪戰鎮壓。任職期間無法執行外派任務。
                  </div>
                </div>

                <div className="grid grid-cols-2 gap-2">
                  <button
                    onClick={() => appointRole(selectedSlave.id, selectedSlave.role === 'maid' ? 'none' : 'maid')}
                    className={`py-3 rounded text-xs font-bold tracking-widest transition-all border shadow-sm active:scale-95 flex items-center justify-center ${
                      selectedSlave.role === 'maid'
                        ? 'bg-blue-950/60 text-blue-400 border-blue-700 font-black shadow-[0_0_10px_rgba(29,78,216,0.3)]'
                        : 'bg-gray-800 text-gray-400 border-gray-600 hover:bg-gray-700'
                    }`}
                  >
                    {selectedSlave.role === 'maid' ? '✓ 內務傭人' : '指派為傭人'}
                  </button>
                  <button
                    onClick={() => appointRole(selectedSlave.id, selectedSlave.role === 'security' ? 'none' : 'security')}
                    className={`py-3 rounded text-xs font-bold tracking-widest transition-all border shadow-sm active:scale-95 flex items-center justify-center ${
                      selectedSlave.role === 'security'
                        ? 'bg-purple-950/60 text-purple-400 border-purple-700 font-black shadow-[0_0_10px_rgba(126,34,206,0.3)]'
                        : 'bg-gray-800 text-gray-400 border-gray-600 hover:bg-gray-700'
                    }`}
                  >
                    {selectedSlave.role === 'security' ? '✓ 商會保全' : '指派為保全'}
                  </button>
                </div>
              </div>
            )}
