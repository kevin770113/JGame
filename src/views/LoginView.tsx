...
export default function LoginView() {
  ...
  const handleAnonLogin = async () => {
    setLoading(true);
    setSysMessage({ text: t('login.msg_connecting', '［系統］正在與深淵建立無名連結...'), type: 'success' });
    const { error } = await supabase.auth.signInAnonymously();
    if (error) {
      setSysMessage({ text: t('login.msg_reject', { msg: error.message, defaultValue: `［拒絕］${error.message}` }), type: 'error' });
      setLoading(false);
    }
  };
  ...
  const handleEmailLogin = async (isSignUp: boolean) => {
    if (!email || !password) {
       setSysMessage({ text: t('login.msg_empty', '［警告］請提供完整的血脈印記（信箱與密碼）。'), type: 'error' });
       return;
    }
    setLoading(true);
    setSysMessage({ text: t('login.msg_verifying', '［系統］驗證血脈中...'), type: 'success' });
    ...
    if (error) {
      ...
      setSysMessage({ text: t('login.msg_signup_success', '［系統］血脈註冊成功，請前往您的信箱點擊驗證信。'), type: 'success' });
    }
    setLoading(false);
  };
    ...
    <div className="w-full min-h-[100dvh] flex flex-col items-center justify-center p-4 bg-dark-bg text-gray-200 animate-fade-in relative">
      ...
      <div className="max-w-sm w-full bg-gray-900 border border-gray-700 rounded-lg p-6 shadow-2xl flex flex-col gap-6 relative overflow-hidden z-10">
         ...
         <div className="text-center mt-2">
           <h1 className="text-2xl font-bold text-gray-200 tracking-widest mb-2">{t('game.title_zh', '戰火中的血統商人')}</h1>
           <p className="text-xs text-gray-500">{t('login.subtitle', '深淵的入口已開啟，請宣告您的降臨方式。')}</p>
         </div>
         ...
         <div className="flex flex-col gap-4">
            ...
            >
              {loading ? t('login.btn_loading', '［連線中...］') : t('login.btn_anon', '［簽署無名血契］(免註冊快速遊玩)')}
            </button>

            <div className="flex items-center gap-3 my-2">
              <div className="h-px bg-gray-800 flex-1"></div>
              <span className="text-2xs text-gray-600 font-bold tracking-widest">{t('login.or_official', '或使用正式印記')}</span>
              <div className="h-px bg-gray-800 flex-1"></div>
            </div>
            ...
            <div className="flex flex-col gap-3">
              <input 
                type="email" 
                placeholder={t('login.ph_email', '信箱地址')}
                value={email} 
              ...
              <input 
                type="password" 
                placeholder={t('login.ph_password', '密碼 (至少 6 碼)')}
                value={password} 
