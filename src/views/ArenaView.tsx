  ...
  return (
    <div className="w-full flex flex-col gap-4 pb-10 animate-fade-in relative z-10">
      <div className="flex justify-between items-center border-b border-gray-800 pb-2">
        <div>
          <h2 className="text-xl font-bold text-red-600/90 tracking-widest">{t('arena.title', '［血腥角鬥場］')}</h2>
          <p className="text-2xs text-gray-500 mt-1">{t('arena.desc', '帝國高層的殘酷娛樂。贏得勝利以獲取資金與威望。')}</p>
        </div>
        <button onClick={() => navigate('Town', 'Main')} className="whitespace-nowrap shrink-0 px-3 py-1.5 bg-gray-900 border border-gray-700 hover:bg-gray-800 text-gray-400 font-bold rounded text-xs transition-colors shadow-sm tracking-widest">
          {t('ui.return_town', '［返回城鎮］')}
        </button>
      </div>

      <div className="bg-black/40 p-4 sm:p-5 rounded-lg border border-red-900/30 shadow-xl flex flex-col gap-5">
        
        <div className="flex flex-col gap-2">
          <label className="text-xs text-red-500/80 font-bold tracking-widest border-l-2 border-red-600/80 pl-2">{t('arena.select_fighter', '［指派上陣鬥士］')}</label>
