...
export const createCombatSlice: StateCreator<GameStore, [], [], any> = (set, get) => ({
  executeArenaBattle: (slaveId: string, npcId: string) => {
    ...
    // ★ 核心升級：雙軌制多語系日誌生成，徹底繞過字典檔缺失問題
    addLog(0, 'log.arena_enter', { slave: slave.name, npc: npc.name }, 
      isEn ? `[System] ${slave.name} enters the arena, facing ${npc.name}.` : `［系統］${slave.name} 踏入賽場，迎戰 ${npc.name}。`, 'system');

    if (slave.race === '精靈') addLog(0, 'log.skill_elf', { slave: slave.name }, 
      isEn ? `[Wind's Blessing] ${slave.name} moves like the wind, striking first with massive damage!` : `［風之眷顧］${slave.name} 身形如風奪得先手，首擊傷害大幅增幅！`, 'skill');
    if (slave.race === '半獸人') addLog(0, 'log.skill_orc', { slave: slave.name }, 
      isEn ? `[Frenzied Blood] ${slave.name} ignites their fury, sacrificing defense for devastating strikes!` : `［狂熱戰血］${slave.name} 點燃了怒火，放棄防禦以換取更具破壞力的重擊！`, 'skill');
    if (slave.race === '矮人') addLog(0, 'log.skill_dwarf', { slave: slave.name }, 
      isEn ? `[Rocky Physique] ${slave.name}'s muscles harden like bedrock, becoming nearly impenetrable!` : `［堅岩體魄］${slave.name} 展現了北地體格，肌肉宛如磐石般堅不可摧！`, 'skill');
    if (slave.race === '龍族') addLog(0, 'log.skill_dragon', { slave: slave.name }, 
      isEn ? `[Dragon's Aura] ${slave.name} releases a supreme aura, boosting all stats and gaining massive damage reduction!` : `［真龍威壓］${slave.name} 釋放了上位威壓，全屬性增幅並獲得巨額減傷！`, 'skill');
    if (slave.race === '人類') addLog(0, 'log.skill_human', { slave: slave.name }, 
      isEn ? `[Desperate Will] ${slave.name} harbors survival potential, ready to erupt in power when near death.` : `［絕境意志］${slave.name} 懷抱求生潛能，瀕死狀態下攻擊力將大幅爆發。`, 'skill');
    if (slave.race === '不死族') addLog(0, 'log.skill_undead', { slave: slave.name }, 
      isEn ? `[Undying Bones] ${slave.name} emanates an aura of death, siphoning the enemy's life force with each strike.` : `［枯骨不朽］${slave.name} 散發著死氣，其攻擊能精準汲取敵人的生命力。`, 'skill');
    ...
    while (sHp > 0 && nHp > 0 && round <= 50) {
      ...
      const slaveAction = () => {
        ...
        if (slave.race === '人類' && sHp < sHpMax * 0.4 && !humanUnstoppable) { 
            ...
            addLog(round, 'log.human_unstoppable', { slave: slave.name }, 
              isEn ? `[Desperate Will] ${slave.name} erupts with a strong desire to survive, greatly increasing attack power!` : `［絕境意志］${slave.name} 爆發強烈的求生欲，攻擊力極大幅提升！`, 'skill'); 
        }
        ...
        if (Math.random() < nDodgeRate) {
            addLog(round, 'log.npc_dodge', { npc: npc.name, slave: slave.name }, 
              isEn ? `[Dodge] ${npc.name} narrowly evaded ${slave.name}'s lethal strike!` : `［閃避］${npc.name} 驚險地避開了 ${slave.name} 的致命一擊！`, 'skill');
        } else {
            ...
            if (Math.random() < sCritRate) {
                ...
                addLog(round, 'log.slave_crit', { slave: slave.name }, 
                  isEn ? `[Critical Hit] ${slave.name} seized a fleeting opening, dealing 1.5x critical damage!` : `［爆擊］${slave.name} 抓住了轉瞬即逝的破綻，造成 1.5 倍爆擊傷害！`, 'damage');
            }
            nHp -= dmg; nHp = Math.max(0, nHp);
            addLog(round, 'log.slave_atk', { slave: slave.name, npc: npc.name, dmg }, 
              isEn ? `${slave.name} attacks, dealing ${dmg} damage to ${npc.name}.` : `${slave.name} 發動攻擊，對 ${npc.name} 造成 ${dmg} 點傷害。`, 'damage', dmg);
            if (slave.race === '不死族') { 
                const heal = Math.floor(dmg * 0.15); 
                if (heal > 0) { 
                    sHp = Math.min(sHpMax, sHp + heal); 
                    addLog(round, 'log.undead_heal', { slave: slave.name, heal }, 
                      isEn ? `[Undying Bones] ${slave.name} absorbed ${heal} life force.` : `［枯骨不朽］${slave.name} 吸收了 ${heal} 點生命力。`, 'heal', heal); 
                } 
            }
        }
      };
      const npcAction = () => {
        ...
        if (Math.random() < sDodgeRate) {
            addLog(round, 'log.slave_dodge', { slave: slave.name, npc: npc.name }, 
              isEn ? `[Dodge] ${slave.name} nimbly evaded ${npc.name}'s attack!` : `［閃避］${slave.name} 靈巧地避開了 ${npc.name} 的攻擊！`, 'skill');
        } else {
            let dmg = Math.max(1, nAtk - sDef); 
            if (slave.race === '矮人') { 
                dmg = Math.max(1, dmg - 5); 
                addLog(round, 'log.dwarf_defend', { slave: slave.name }, 
                  isEn ? `[Natural Defense] ${slave.name}'s [Rocky Physique] forcefully mitigated 5 points of lethal damage!` : `［天賦防禦］${slave.name} 的【堅岩體魄】使其硬生生抵擋並折抵了 5 點致命傷害！`, 'skill'); 
            }
            ...
            if (Math.random() < nCritRate) {
                dmg = Math.floor(dmg * 1.5);
                addLog(round, 'log.npc_crit', { npc: npc.name, slave: slave.name }, 
                  isEn ? `[Critical Hit] ${npc.name}'s ferocious assault dealt 1.5x critical damage to ${slave.name}!` : `［爆擊］${npc.name} 的攻勢異常兇猛，對 ${slave.name} 造成 1.5 倍爆擊傷害！`, 'damage');
            }
            sHp -= dmg; sHp = Math.max(0, sHp);
            addLog(round, 'log.npc_atk', { npc: npc.name, slave: slave.name, dmg }, 
              isEn ? `${npc.name} swings their weapon, dealing ${dmg} damage to ${slave.name}.` : `${npc.name} 揮舞武器，對 ${slave.name} 造成 ${dmg} 點傷害。`, 'damage', dmg);
            if (slave.race === '半獸人') { 
                orcStack++; 
                addLog(round, 'log.orc_stack', { slave: slave.name }, 
                  isEn ? `[Talent Stack] ${slave.name} endures the attack, pain fuels their frenzy!` : `［天賦層數］${slave.name} 承受攻擊，痛苦激發狂暴！`, 'skill'); 
            }
        }
      };
    ...
    if (isWin) {
      newWins++;
      addLog(round - 1, 'log.arena_win', { slave: slave.name }, 
        isEn ? `[Result] ${slave.name} stood until the end, achieving victory.` : `［結算］${slave.name} 屹立到了最後，取得勝利。`, 'system');
      ...
      addLog(round - 1, 'log.food_reward', { foodReward }, 
        isEn ? `[Bonus Loot] Scavenged ${foodReward} units of rations from the opponent.` : `［戰利品附加］在對手身上搜刮到口糧 ${foodReward} 單位。`, 'system');

      if (charismaBonus > 1) {
          addLog(round - 1, 'log.charisma_bonus', { finalRewardGold }, 
            isEn ? `[Charisma Bonus] Defeated a high-charisma opponent, earning bonus gold for a total of ${finalRewardGold}!` : `［戰利品加成］擊敗高魅力對手，資金獲得額外加成，共計獲取 ${finalRewardGold}！`, 'system');
      }
      ...
      if (netWins > 0 && netWins % 5 === 0) {
        ...
        addLog(round - 1, 'log.stat_up', { slave: slave.name, netWins, stat: nameMap[picked] }, 
          isEn ? `Breakthrough! ${slave.name} accumulated ${netWins} net wins, permanently increasing [${nameMap[picked]}] by 1!` : `歷練突破！${slave.name} 累積淨勝場達 ${netWins} 場，【${nameMap[picked]}】永久提升 1 點！`, 'skill');
      }
      ...
      set((s: GameStore) => {
        ...
        // ★ 核心修復：特質前綴僅寫入 id，確保 npc.name 純淨，使高亮染色永不失效
      ...
      newStamina = 0; 
      isInjuredNow = true; 
      addLog(round - 1, 'log.arena_lose', { slave: slave.name }, 
        isEn ? `[Result] ${slave.name} took heavy damage and collapsed, exhausting all stamina and gaining the [Injured] state!` : `［結算］${slave.name} 遭受重創倒地，體力耗盡並陷入【負傷】狀態！`, 'system');
    }
  ...
  executeAbyssBattle: (slaveId: string) => {
    ...
    addLog(0, 'log.abyss_enter', { slave: slave.name, floor, enemy: enemy.name }, 
      isEn ? `[System] ${slave.name} enters Abyss Floor ${floor}, facing ${enemy.name}.` : `［系統］${slave.name} 踏入深淵第 ${floor} 階，迎戰 ${enemy.name}。`, 'system');
    addLog(0, 'log.abyss_quote', { quote: enemy.quote }, 
      isEn ? `"${enemy.quote}"` : `「${enemy.quote}」`, 'system');
    ...
    while (sHp > 0 && nHp > 0 && round <= 50) {
      ...
      const slaveAction = () => {
        ...
        const nDodgeRate = Math.min(0.20, (nLuck / 100) * 0.20);
        if (Math.random() < nDodgeRate) {
            addLog(round, 'log.npc_dodge', { npc: enemy.name, slave: slave.name }, 
              isEn ? `[Dodge] ${enemy.name} narrowly evaded ${slave.name}'s lethal strike!` : `［閃避］${enemy.name} 驚險地避開了 ${slave.name} 的致命一擊！`, 'skill');
        } else {
            let dmg = Math.floor(Math.max(1, atkPower - nDef) * dmgMulti); 
            const sCritRate = Math.min(0.30, (sLuck / 100) * 0.30);
            if (Math.random() < sCritRate) {
            ...
            nHp -= dmg; nHp = Math.max(0, nHp);
            addLog(round, 'log.slave_atk', { slave: slave.name, npc: enemy.name, dmg }, 
              isEn ? `${slave.name} attacks, dealing ${dmg} damage to ${enemy.name}.` : `${slave.name} 發動攻擊，對 ${enemy.name} 造成 ${dmg} 點傷害。`, 'damage', dmg);
            if (slave.race === '不死族') { 
                const heal = Math.floor(dmg * 0.15); 
                if (heal > 0) { 
                    sHp = Math.min(sHpMax, sHp + heal); 
      ...
      const npcAction = () => {
        ...
        const sDodgeRate = Math.min(0.20, (sLuck / 100) * 0.20);
        if (Math.random() < sDodgeRate) {
            addLog(round, 'log.slave_dodge', { slave: slave.name, npc: enemy.name }, 
              isEn ? `[Dodge] ${slave.name} nimbly evaded ${enemy.name}'s attack!` : `［閃避］${slave.name} 靈巧地避開了 ${enemy.name} 的攻擊！`, 'skill');
        } else {
            let dmg = Math.max(1, nAtk - sDef); 
            if (slave.race === '矮人') { 
                dmg = Math.max(1, dmg - 5); 
            ...
            if (Math.random() < nCritRate) {
                dmg = Math.floor(dmg * 1.5);
                addLog(round, 'log.npc_crit', { npc: enemy.name, slave: slave.name }, 
                  isEn ? `[Critical Hit] ${enemy.name}'s ferocious assault dealt 1.5x critical damage to ${slave.name}!` : `［爆擊］${enemy.name} 的攻勢異常兇猛，對 ${slave.name} 造成 1.5 倍爆擊傷害！`, 'damage');
            }
            sHp -= dmg; sHp = Math.max(0, sHp);
            addLog(round, 'log.npc_atk', { npc: enemy.name, slave: slave.name, dmg }, 
              isEn ? `${enemy.name} swings their weapon, dealing ${dmg} damage to ${slave.name}.` : `${enemy.name} 揮舞武器，對 ${slave.name} 造成 ${dmg} 點傷害。`, 'damage', dmg);
            if (slave.race === '半獸人') { 
                orcStack++; 
                addLog(round, 'log.orc_stack', { slave: slave.name }, 
                  isEn ? `[Talent Stack] ${slave.name} endures the attack, pain fuels their frenzy!` : `［天賦層數］${slave.name} 承受攻擊，痛苦激發狂暴！`, 'skill'); 
    ...
    if (isWin) {
      newWins++;
      addLog(round - 1, 'log.abyss_win', { slave: slave.name }, 
        isEn ? `[Result] ${slave.name} crushed the Abyss's obstacle and successfully advanced!` : `［結算］${slave.name} 擊潰了深淵的阻礙，成功晉級！`, 'system');
      ...
      if (charismaBonus > 1) {
          addLog(round - 1, 'log.abyss_charisma_bonus', { finalRewardGold }, 
            isEn ? `[Charisma Bonus] Defeated a high-charisma Abyss Lord, earning bonus gold for a total of ${finalRewardGold}!` : `［戰利品加成］擊破高魅力淵主，資金獲得額外加成，共計獲取 ${finalRewardGold}！`, 'system');
      }
      ...
      newStamina = 0; 
      isInjuredNow = true; 
      addLog(round - 1, 'log.abyss_lose', { slave: slave.name }, 
        isEn ? `[Result] ${slave.name} collapsed, mercilessly consumed by the Abyss and gaining the [Injured] state!` : `［結算］${slave.name} 不支倒地，被深淵無情吞噬並陷入【負傷】狀態！`, 'system');
    }
