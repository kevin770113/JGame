...
export const generateDailyMissions = (): Mission[] => {
  ...
  const getNameEn = () => `${actions[Math.floor(Math.random() * actions.length)]} ${targets[Math.floor(Math.random() * targets.length)]}`;
  const getNameZh = () => `${actions[Math.floor(Math.random() * actions.length)]}${targets[Math.floor(Math.random() * targets.length)]}`;

  for (let i = 0; i < Math.floor(Math.random() * 2) + 3; i++) {
    missions.push({ 
      id: `m-grn-${baseId}-${i}`, title: isEn ? `[Routine] ${getNameEn()}` : `［常規］【${getNameZh()}】`, rank: '翠綠', requiredPhases: 1, staminaCost: 20, stressGain: 10, reward: 250 + Math.floor(Math.random() * 100), 
      description: isEn ? 'Routine field operation, mild intensity.' : '常規安全外派，勞動性質溫和。', successRate: 1.0, obedienceReward: 1 
    });
  }
  for (let i = 0; i < Math.floor(Math.random() * 2) + 1; i++) {
    missions.push({ 
      // ★ V2.11.0 修復：徹底移除硬編碼的［進階］，支援中英雙語標題與動態拼接
      id: `m-blu-${baseId}-${i}`, title: isEn ? `[Advanced] ${getNameEn()}` : `［進階］【${getNameZh()}】`, rank: '蔚藍', requiredPhases: 2, staminaCost: 35, stressGain: 15, reward: 800 + Math.floor(Math.random() * 200), 
  ...
  if (Math.random() > 0.7) {
    missions.push({ 
      id: `m-pur-${baseId}`, title: isEn ? `[Spec-Op] ${getNameEn()}` : `［特化］【${getNameZh()}】`, rank: '紫色', requiredPhases: 3, staminaCost: 50, stressGain: 25, reward: 2000 + Math.floor(Math.random() * 400), 
  ...
  if (Math.random() > 0.8) {
    missions.push({ 
      id: `m-gld-${baseId}`, title: isEn ? `[Legendary] ${getNameEn()}` : `［傳說］【${getNameZh()}】`, rank: '黃金', requiredPhases: 5, staminaCost: 70, stressGain: 40, reward: 3500 + Math.floor(Math.random() * 800), 
