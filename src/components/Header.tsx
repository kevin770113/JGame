...
export default function Header() {
  ...
  // ★ 修正：極簡化環境顯示，移除「環境」贅字，濃縮為格式：乾淨(xx%)
  ...
  return (
    // ★ 縮小 Padding 與 Gap，並強迫在手機端維持更緊湊的排列
    <header className="bg-gray-900 border-b border-gray-700 p-2 sm:p-3 flex flex-col md:flex-row justify-between items-center text-xs shadow-md z-10 gap-2 select-none">
      ...
      <div className="flex flex-wrap gap-2 sm:gap-3 items-center justify-center">
        ...
        <button 
          onClick={processTurn}
          className="bg-gray-800 hover:bg-gray-700 text-gray-300 px-2 py-1 sm:px-3 sm:py-1 rounded font-bold transition-all shadow-sm border border-gray-600 active:scale-95 tracking-widest ml-1"
        >
          推進
        </button>
      </div>
