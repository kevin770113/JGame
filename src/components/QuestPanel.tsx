...
export default function QuestPanel() {
  ...
  return (
    <div className="fixed left-0 top-1/4 Object z-50 flex items-start animate-fade-in pointer-events-none">
              ...
              {activeQuests.map(qId => {
                ...
                return (
                  <div key={qId} className="bg-gray-950 p-2.5 rounded border border-gray-800 relative overflow-hidden shadow-inner">
                    ...
                    <h4 className="text-2xs font-bold text-gray-300 mb-1 pl-1 tracking-wide">{qData.title}</h4>
                    <p className="text-3xs text-gray-500 leading-normal pl-1">{qData.description}</p>
                  </div>
                );
              })}
            </div>
